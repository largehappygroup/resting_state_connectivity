...
format_for_fix(){
      ...
      flirt -in "$1/BrainExtractionMask.nii.gz" \
            -ref "$1/reg/example_func.nii.gz" \
            -out "$1/mask.nii.gz" \
            -applyxfm \
            -usesqform
      ...
      flirt -in "$1/reg/highres.nii.gz" \
            -ref "$1/reg/example_func.nii.gz" \
            -out "$1/reg/highres2example_func" \
            -omat "$1/reg/highres2example_func.mat"
...
find "./data/midprocess/" -maxdepth 1 -type d | while read -r folder; do
      foldername="${folder:18}"
      echo "$foldername"
      if [[ "$foldername" =~ "out_001" ]] || [[ "$foldername" =~ "out_003" ]]; then
            echo "$foldername"
            datadir="./data/midprocess/$foldername"
            perform_ica "$datadir"
