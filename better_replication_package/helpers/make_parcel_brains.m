function make_parcel_brains(seed, sig_idx, stats, empty_brain, brain_idx, parcels, nii_template)
    ...
    if sum(new_brain_neg(:)) ~= 0
        filename = sprintf("../sample_results/parcel_map_seed%d_neg_q01",seed);
        write_nii_cc(nii_template, new_brain_neg, filename);
        compress_file = sprintf("gzip ../sample_results/parcel_map_seed%d_neg_q01.nii", seed);
        system(compress_file);
    if sum(new_brain_pos(:)) ~= 0
        filename = sprintf("../sample_results/parcel_map_seed%d_pos_q01",seed);
        write_nii_cc(nii_template, new_brain_pos, filename);
        compress_file = sprintf("gzip ../sample_results/parcel_map_seed%d_pos_q01.nii", seed);
        system(compress_file);
    end
