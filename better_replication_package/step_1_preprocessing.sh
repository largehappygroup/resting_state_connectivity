...
find ./data/raw/ -maxdepth 1 -type d | while read -r folder; do
      foldername="${folder:11}"
      echo $foldername
      if [[ "$foldername" =~  ^[0-9]{3}_[0-9]{3}$ ]]; then
            mkdir "./data/midprocess/out_$foldername" 
            echo "Processing folder $foldername"
            if [[ "$foldername" =~ ^101_[0-9]{3}$ ]]; then
                  ANATFILE="./data/raw/$foldername/t1spgr_156sl.nii"
                  FUNCFILE="./data/raw/$foldername/tprun_01.nii"

            elif [[ "$foldername" =~ ^002_[0-9]{3}$ ]]; then
                  ANATFILE="./data/raw/$foldername/t1spgr_208sl.nii"
                  FUNCFILE="./data/raw/$foldername/tprun_01.nii"
            else
                  ANATFILE="./data/raw/$foldername/ht1spgr_208sl.nii"
                  FUNCFILE="./data/raw/$foldername/utrun_01.nii"
            fi
            
            OUT="./data/midprocess/out_$foldername"
            FINAL_OUT="$foldername"
